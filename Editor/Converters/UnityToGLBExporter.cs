...
namespace VoyageVoyage
{
    ...
    public class UnityToGLBExporter
    {
        ...
        static private List<TextureImporter> modifiedImporters = new List<TextureImporter>(16);

        static public void RevertImporters()
        {
            foreach (var importer in modifiedImporters)
            {
                importer.isReadable = false;
            }
            modifiedImporters.Clear();
            AssetDatabase.Refresh();
        }
        ...
        static private int AddTextureToImages(GltfContext context, Texture2D texture)
        {
            ...
            else
            {
                ...
                if (!texture.isReadable)
                {
                    ...
                    if (importer == null)
                    {
                        Debug.LogError("Unable to get TextureImporter for: " + texture.name);
                        throw new Exception($"Could not convert texture {texture.name}");
                    }

                    importer.isReadable = true;
                    modifiedImporters.Add(importer);
